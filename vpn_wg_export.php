...
/*
 * vpn_wg_export.php
 * Ultimate pfSense WG Peer Export (v0.4.3 - Secure Edition)
...
if (isset($_POST['action']) && $_POST['action'] === "add_peer") {
    ...
    // Sanitize inputs against newline/carriage return injection
    $tun_name = preg_replace('/[\r\n]/', '', $_POST['tun'] ?? '');
    $publickey = preg_replace('/[\r\n]/', '', trim($_POST['publickey'] ?? ''));
    $assigned_ip = preg_replace('/[\r\n]/', '', trim($_POST['assignedip'] ?? ''));
    $descr = preg_replace('/[\r\n]/', '', trim($_POST['descr'] ?? 'New Peer'));
    $presharedkey = preg_replace('/[\r\n]/', '', $_POST['presharedkey'] ?? '');
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === "get_conf_data" && isset($_GET['peer_idx'])) {
        $peer_idx = preg_replace('/[^0-9]/', '', $_GET['peer_idx']);
        $a_tunnels = get_wg_config_array('tunnel');
        $a_peers = get_wg_config_array('peer');
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-sm btn-info" onclick="openExportModal(<?=$idx;?>, <?=htmlspecialchars(json_encode($display_desc), ENT_QUOTES, 'UTF-8');?>)">
...
$(document).ready(function() {
    ...
    // XSS FIX: safely passing string payload into JavaScript variable via json_encode
    let autoName = <?=json_encode($a_peers[$_GET['provision_idx']]['descr'] ?? 'Peer');?>;
